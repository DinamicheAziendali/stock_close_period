...
class XlsxStockClosePeriodProduct(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.merge_range(
            ...
            i,
            9,
            _("Close period - %(period_name)s - %(period_date)s - %(period_company)s")
            % {
                "period_name": stock_close_period.name,
                "period_date": stock_close_period.close_date.strftime("%d/%m/%Y"),
                "period_company": stock_close_period.company_id.name,
            },
            title_style,
        )
        i += 1
